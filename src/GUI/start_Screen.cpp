...
void StartScreen::createButtons() {
    ...
    // SETTINGS Button
    buttons.push_back(StartButton(
        1, glm::vec2(centerX, centerY + 20),
        glm::vec2(300, 80),
        glm::vec3(0.2f, 0.5f, 0.9f),      // Blue
    ...
    // EXIT Button
    buttons.push_back(StartButton(
        2, glm::vec2(centerX, centerY + 120),
        glm::vec2(300, 80),
...
int StartScreen::handleButtonClick() {
    ...
    for (const auto& button : buttons) {
        if (button.isMouseOver(mousePos)) {
            mousePressed = false;  // Consume the click
            return button.id;
        }
    }
    mousePressed = false;
...
void StartScreen::renderButton(const StartButton& button) {
    ...
    // Render button text (using simple text rendering)
    renderText(button.label, button.position.x - 50, button.position.y - 15, 1.5f, button.textColor);
}

// ===================== RENDER TEXT (SIMPLE - JUST DISPLAYS TEXT INFO) =====================
void StartScreen::renderText(const std::string& text, float x, float y, float scale, glm:: vec3 color) {
    // Simple text representation using rectangles as placeholders
    // In a real implementation, you'd use a font rendering library
    // For now, this is a placeholder that shows text positioning
}
...
void StartScreen::renderTitle() {
    ...
    renderText("FPS SHOOTER GAME", screenWidth / 2 - 150, 50, 2.0f, glm::vec3(1.0f, 1.0f, 0.0f));
}
